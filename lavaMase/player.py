...
if TYPE_CHECKING:
    from disnake import TextChannel, Thread, VoiceChannel, StageChannel
...
class Player(disnake.VoiceProtocol):
    ...
    auto_queue: :class:`~Queue`
        The auto_queue associated with this player.
        This queue holds tracks that are recommended by the AutoPlay feature.
    """
    ...
    def __init__(
        ...
        self._autoplay: lavaMase.AutoPlayMode = lavaMase.AutoPlayMode.partial
        ...
        self._inactivity_wait: int | None = self._node.inactive_timeout

        self._paused_task: asyncio.Task[bool] | None = None
        self._paused_wait: int | None = self._node.resume_timeout

        self._namespace: dict[str, any] = defaultdict()

    def dispatch_message_update(self) -> None:
        if not self._current:
            return

        self.client.dispatch("Mase_message_update", self)

    @property
    def namespace(self) -> ExtrasNamespace:
        return ExtrasNamespace(self._namespace)

    def add_to_namespace(self, value: dict[str, any], /) -> None:
        if not isinstance(value, dict):
            raise TypeError(f"Expected a dict, got {type(value)}")

        for key, _value in dict(value).items():
            if self._namespace.get(key) and _value is None:
                del self._namespace[key]
                continue

            self._namespace[key] = _value
    ...
    def _inactivity_task_callback(self, task: asyncio.Task[bool]) -> None:
        ...
        self.client.dispatch("Mase_inactive_player", self)
        logger.debug(f'Dispatched "on_Mase_inactive_player" for Player {self._guild.id}.')
    ...
    def inactivity_cancel(self) -> None:
    ...
    def inactivity_start(self) -> None:
        if self._inactivity_wait is not None and self._inactivity_wait > 0:
            self._inactivity_task = asyncio.create_task(self._runner(self._inactivity_wait))
            self._inactivity_task.add_done_callback(self._inactivity_task_callback)

    def track_start(self) -> None:
        self.inactivity_cancel()

    async def auto_play_event(self, payload: TrackEndEventPayload) -> None:
        if self._autoplay is lavaMase.AutoPlayMode.disabled:
        ...
        if self.node.status is not lavaMase.NodeStatus.CONNECTED:
        ...
        if self.queue.mode is lavaMase.QueueMode.loop:
            await self._do_partial(history=False)

        elif self.queue.mode is lavaMase.QueueMode.loop_all:
            await self._do_partial()

        elif self._autoplay is lavaMase.AutoPlayMode.partial or self.queue:
            await self._do_partial()
    ...
    # TODO: Recommendations maybe later
    ...
    def inactive_timeout(self) -> int | None:
        ...
        :func:`on_Mase_inactive_player` event.
        ...
        - The default countdown for all players is set on :class:`~Node`.

        This property can be set with a valid ``int`` of seconds to wait before dispatching the
        :func:`on_Mase_inactive_player` event or ``None`` to remove the timeout.
        ...
        - See: :class:`~Node`
        - See: :func:`on_Mase_inactive_player`
        """
    ...
    @property
    def autoplay(self) -> lavaMase.AutoPlayMode:
        """A property which returns the :class:`AutoPlayMode` the player is currently in.

        This property can be set with any :class:`AutoPlayMode` enum value.


        .. versionchanged:: 3.0.0

            This property now accepts and returns a :class:`AutoPlayMode` enum value.
        """
        return self._autoplay

    @autoplay.setter
    def autoplay(self, value: Any) -> None:
        if not isinstance(value, lavaMase.AutoPlayMode):
            raise ValueError("Please provide a valid 'AutoPlayMode' to set.")
    ...
    def current(self) -> Playable | None:
        """Returns the currently playing :class:`~Playable` or None if no track is playing."""
    ...
    def filters(self) -> Filters:
        """Property which returns the :class:`~Filters` currently assigned to the Player.

        See: :meth:`~Player.set_filters` for setting the players filters.
    ...
    def paused(self) -> bool:
        ...
        See: :meth:`pause` and :meth:`play.py` for setting the paused status.
        """
        return self._paused
    ...
    def playing(self) -> bool:
        ...
        """
        return self._connected and self._current is not None and not self._paused

    @property
    def position(self) -> int:
        """Returns the position of the currently playing :class:`~Playable` in milliseconds.

        This property relies on information updates from Lavalink.

        In cases there is no :class:`~Playable` loaded or the player is not connected,
    ...
    async def update_event(self, payload: PlayerUpdateEventPayload) -> None:
    ...
    async def _dispatch_voice_update(self) -> None:
        ...
        try:
            await self.node.update(self.guild.id, data=request)
    ...
    async def connect(
        ...
        Pass the :class:`Player` to ``cls=`` in :meth:`disnake.VoiceChannel.connect`.
        ...
        self.node.players[self._guild.id] = self
    ...
    async def play(
        ...
        *,
        ...
        """Play the provided :class:`~Playable`.
        ...
        start: int
            ...
            Defaults to ``None`` which means this track will play.py until the very end.
        volume: Optional[int]
        ...
        add_history: Optional[bool]
            ...
            :class:`Queue` history, if loading the track was successful. If ``False`` this track will not be
        ...
        filters: Optional[:class:`~Filters`]
            An Optional[:class:`~Filters`] to apply when playing this track. Defaults to ``None``.
            If this is ``None`` the currently set filters on the player will be applied.
        clear_auto: Optional[bool]
            If user wants to clear the ``AutoPlay Queue`` after the track has been played. Defaults to `False`
        ...
        if volume and self._volume != volume:
            self._volume = volume
        ...
        try:
            await self.node.update(self.guild.id, data=request, replace=replace)
    ...
    def paused_cancel(self) -> None:
        if self._paused_task:
            try:
                self._paused_task.cancel()
            except Exception as e:
                _ = e
                pass
    ...
    def paused_start(self) -> None:
        if self._paused_wait > 0:
            self._paused_task = asyncio.create_task(self._runner(self._paused_wait))
            self._paused_task.add_done_callback(self._inactivity_task_callback)
    ...
    async def set_filters(self, filters: Filters | None = None, /, *, seek: bool = True) -> None:
        """Set the :class:`Filters` on the player.
    ...
    async def disconnect(self, **kwargs: Any) -> None:
        """Disconnect the player from the current voice channel and remove it from the :class:`~Node`.

        This method will cause any playing track to stop and potentially trigger the following events:

            - ``on_Mase_track_end``
            - ``on_Mase_websocket_closed``
    ...
    async def skip(self, *, force: bool = True) -> Playable | None:
        ...
        ----------
        force: bool
            Whether the track should skip looping, if :class:`Queue` has been set to loop.
            Defaults to ``True``.
        ...
        await self.node.update(self.guild.id, data=request, replace=True)
        self.dispatch_message_update()
    ...
    async def destroy(self) -> None:
        ...
        self._invalidate()
        player: Player | None = self.node.players.pop(self.guild.id, None)
        self.node.client.dispatch("Mase_player_destroyed", player)

        if player:
            try:
                await self.node.destroy(self.guild.id)
            except LavalinkException:
                pass
